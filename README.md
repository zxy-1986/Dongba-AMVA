...
See [`data/README.md`](data/README.md) for the JSON schema.
...
| Boundary decoding weight | 1.0 |
| Maximum candidate segment length | 10 fragments |
| Text tokenizer max length | 52 |
...
| SplitAcc | Page-EM | N<M Acc |
|---:|---:|---:|
| 63.83 | 40.35 | 51.12 |

## Reproducibility notes

1. Use the exported `pages.json` rather than re-running a new random split.
2. The released split is book-disjoint to avoid manuscript-level style leakage.
3. The candidate-segment upper bound is fixed at 10, matching the accepted experiments.
4. For B0/B1, boundary scores are fully disabled in both training and decoding.
5. CABP outputs boundary logits; sigmoid is applied only when the boundary probability
